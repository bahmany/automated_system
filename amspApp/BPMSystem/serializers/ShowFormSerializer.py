...
class ShowFormSerializer(DynamicFieldsDocumentSerializer):
    ...
    class Meta:
        ...
        fields = (
            ...
            'positionPic', 'id', 'postDate', 'bpmnName', 'bpmnId', 'prevPerformer', 'formSchema', 'formData','chartTitle','seen')
        depth = 1
    ...
    def get_form_data(self, obj):
        ...
        if 'chartOptions' in obj.formData:
            result['chartOptions'] = obj.formData['chartOptions']
        if 'chartSelected' in obj.formData:
            result['chartSelected'] = obj.formData['chartSelected']
        if 'thisSteps' in obj:
            for itm in obj.thisSteps:
                if str(currentPosition.id) in itm.keys():
                    currentStep = itm[str(currentPosition.id)]
