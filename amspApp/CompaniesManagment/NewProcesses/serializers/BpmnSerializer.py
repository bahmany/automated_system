...
class BpmnSerializer(DocumentSerializer):
    ...
    class Meta:
        ...
        fields = ('xml', 'description', 'user_id',
                  ...
                  'form','processObjs', 'userTasks')


    def create(self, validated_data, **kwargs):
        validated_data['user_id'] = kwargs['request'].user.pk
        if Bpmn.objects.filter(user_id=validated_data['user_id'],company_id=validated_data["company_id"], name=validated_data['name']).count() > 0:
    ...
    def update(self, instance, validated_data):
        datas = Bpmn.objects.filter(user_id=instance['user_id'],company_id=validated_data["company_id"], name=validated_data['name'])
        counts = datas.count()
        if counts == 1:
            for obj in datas:
