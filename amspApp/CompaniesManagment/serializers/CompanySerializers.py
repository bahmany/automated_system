...
class CompanySerializer(serializers.ModelSerializer):
    ...
    #     allow_blank=False
    # )
    # is_it_default = serializers.SerializerMethodField(read_only=True)
    ...
    # def get_is_it_default(self, obj):
    #     return obj
    ...
    def createDepedenciesOfCompany(self, user, newCompany):
        ...
        newMemberPosition = newMemberPosition.create(newMember)
        newSec = {
            'name': _("main office"),
        ...
        positionDocInstance = PositionsDocument.objects.get(positionID = newMemberPosition.id)
        positionPartialUpdate = PositionDocumentSerializer(data = {
            "defaultSec" : newSec.id
        }, instance=positionDocInstance, partial=True)
        positionPartialUpdate.is_valid(raise_exception=True)
        positionPartialUpdate.update(positionDocInstance, positionPartialUpdate.validated_data)
    ...
    def create_default_company_from_user(self, MyUser):
        newComp = {
            ...
            "name":" شرکت پیش فرض  " + MyUser.username,
