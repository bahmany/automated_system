...
class InboxFolderSerializer(DynamicFieldsDocumentSerializer):
    ...
    #     result = super(InboxFolderSerializer, self).update(instance, validated_data)
    ...
    def UpdateCounterStatics(self, oldInboxInstance, newValidatedData):
        if not "folders" in newValidatedData:
            return

        GetStaticsViewSet().getInboxFoldersStatisticsRenewCache()
