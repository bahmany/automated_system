...
from amspApp.Sales.serializers.KhoroojSerializer import HamkaranKhoroojSerializer, KhoroojSMSSerializer, \
    ...
    HamkaranKhoroojSignsSnapshotSerializer
from amspApp.Sales.serializers.SQLHavaleKhoroojToPishfactor import havaleh_khrooj_sql_order_id, get_qotation_by_id, \
    get_qotation_items_by_id, get_qotation_desc_by_id
...
class KhoroojViewSet(viewsets.ModelViewSet):
    ...
    def pishfatorget(self, request, *args, **kwargs):
        if request.user.groups.all().filter(name='group_view_pishfactor').count() == 0:
            return {}
        connection = Connections.objects.get(databaseName="RahkaranDB")
        connection = ConnectionsViewSet().getConnection(connection)
        connection.execute(get_qotation_by_id(kwargs.get("id")))
        ...
        result = {}
        result['pishfator_main'] = sql_res[0]

        connection.execute(get_qotation_items_by_id(kwargs.get("id")))
        sql_res = connection.fetchall()
        sql_res = convert_sqlresultstr_to_valid_str(sql_res)
        sql_res = convert_sqlresultstr_to_valid_numbers(sql_res)
        result['pishfator_items'] = sql_res

        connection.execute(get_qotation_desc_by_id(kwargs.get("id")))
        sql_res = connection.fetchall()
        sql_res = convert_sqlresultstr_to_valid_str(sql_res)
        sql_res = convert_sqlresultstr_to_valid_numbers(sql_res)
        result['pishfator_descs'] = sql_res

        result['pishfator_box'] = {
            'jame_meghdar': query(result['pishfator_items']).sum(lambda x: x['QuotationItemQuantity']),
            'mablaghe_nakhales': result['pishfator_main']['QuotationPrice'],
            'ezafaat': result['pishfator_main']['QuotationAdditionAmount'],
            'kosoor': result['pishfator_main']['QuotationReductionAmount'],
            'malaghe_khales': result['pishfator_main']['QuotationNetPrice'],
        }
    ...
    def retrieve(self, request, *args, **kwargs):
        ...
        for i in result.data['items']:
            i['havaleh_foroosh_tarikh_sh'] = get_date_str(i.get('havaleh_foroosh_tarikh', '')) if i.get(
                'havaleh_foroosh_tarikh', '') != '' else ''
        ...
        connection = ConnectionsViewSet().getConnection(connection)
        for i in result.data['items']:
            if i.get('havaleh_foroosh_id', False):
                item_of_havaleh_foroosh = HamkaranHavaleForooshOrderApprove.objects.filter(
                    havalehForooshLink=i['havaleh_foroosh_id']).first()
                connection.execute(havaleh_khrooj_sql_order_id(item_of_havaleh_foroosh.item['items'][0]['OrderItemID']))
                sql_res = connection.fetchall()
                sql_res = convert_sqlresultstr_to_valid_str(sql_res)
                sql_res = convert_sqlresultstr_to_valid_numbers(sql_res)
                pishfator_number = sql_res[0]['OrderItemSourceNumberRef']
                i['pishfactor_id'] = pishfator_number
        # -----------------------
        return result
    ...
    def signExit(self, request, *args, **kwargs):
        ...
        dt = HamkaranKhoroojSignsSnapshotSerializer(
            data={
                'khoroojSignLink': inst.id,
                'snapshot': {
                    'khorooj': HamkaranKhoroojSerializer(instance=exitInstance).data,
                    'khorooj_items': HamkaranKhroojItemsSerializer(instance=HamkaranKhoroojItems.objects.filter(
                        khoroojLink=str(exitInstance.id)
                    ), many=True).data
                }
            }
        )
...
**** ****""" % (
    ...
    def makeItBatel(self, request, *args, **kwargs):
        ...
        # HamkaranKhoroojSigns.objects.filter(exitsLink=exitInstance)
        ...
        if len(signs) < 2:
            ...
            ess = HamkaranKhoroojSerializer(instance=exit, data={'exp': itt}, partial=True)
