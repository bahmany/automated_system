...
from amspApp.Sales.serializers.SQLHavaleKhoroojToPishfactor import havaleh_khrooj_sql_order_id
...
class HavalehForooshViewSet(viewsets.ModelViewSet):
    ...
    def getVaziat(self, step):
        ...
        if step == 4:
            return "منتظر تایید تولید مدیر عامل یا مدیر مالی"
        if step == 5:
    ...
    def getAggr(self, request, *args, **kwargs):
        ...
        what_is_my_group = \
            query(
                list(request.user.groups.filter(name__contains='group_havalehforoosh').values('name'))
            ).select(lambda x: x.get('name', '')).to_list()
        ss = []
        if len(what_is_my_group) > 0:
            for s in what_is_my_group:
                if len(s.split('_')) == 6:
                    ss.append(int(s.split('_')[5]))
        ...
        if len(havals) > 0:
            ...
            if request.query_params.get('archive',
                                        '') == "true":  # در صورتی که تیک آرشیو زده شود تمامی آیتم ها نماتیش داده می شود
        ...
        for d in data:
            d['mizanAvaliehSefaresh'] = query(
                HamkaranHavaleForooshOrderApprove.objects.filter(
                    havalehForooshLink=d['_id'])[0].item['items']).sum(
            ...
            if len(lastSigns) == 0:
                ...
                ss = query(ss).select(lambda x: int(x)).to_list()
                if d['vaziat_id'] in ss:
                    d['waiting_for_me'] = True
    ...
    def getDetails(self, request, *args, **kwargs):
        ...
        if len(s_1) > 0:
            ...
            s_1[0]['dateOfPost'] = s_1[0]['dateOfPost'] + ".0001" if s_1[0]['dateOfPost'].find('.') == -1 else s_1[0][
                'dateOfPost']
        ...
        if len(s_2) > 0:
            ...
            s_2[0]['dateOfPost'] = s_2[0]['dateOfPost'] + ".0001" if s_2[0]['dateOfPost'].find('.') == -1 else s_2[0][
                'dateOfPost']
        ...
        if len(s_3) > 0:
            ...
            s_3[0]['dateOfPost'] = s_3[0]['dateOfPost'] + ".0001" if s_3[0]['dateOfPost'].find('.') == -1 else s_3[0][
                'dateOfPost']
        ...
        if len(s_4) > 0:
            ...
            s_4[0]['dateOfPost'] = s_4[0]['dateOfPost'] + ".0001" if s_4[0]['dateOfPost'].find('.') == -1 else s_4[0][
                'dateOfPost']
            ...
            if (len(s4_approve_exit)) > 0:
                ...
                s4_approve_exit[0]['dateOfPost'] = s4_approve_exit[0]['dateOfPost'] + ".0001" if s4_approve_exit[0][
                                                                                                     'dateOfPost'].find(
                    '.') == -1 else s4_approve_exit[0]['dateOfPost']
        ...
        if len(s_5) > 0:
            ...
            s_5[0]['dateOfPost'] = s_5[0]['dateOfPost'] + ".0001" if s_5[0]['dateOfPost'].find('.') == -1 else s_5[0][
                'dateOfPost']
        ...
        for i in ApprovesIns:
            for ii in i['item']['items']:
                connection.execute(havaleh_khrooj_sql_order_id(ii['OrderItemID']))
                ...
                pishfator_number = sql_res[0].get('OrderItemSourceNumberRef', 'حواله فروش یافت نشد') if len(
                    sql_res) > 0 else 'حواله فروش یافت نشد'
                """
                این داستان زمانی پیش می آید که
                حواله فروش هایی صادر شده است 
                که پیش فاکتور آن پس از صدور حواله
                ویرایش شده اند
                این غیر مجاز است و نمی توان این کار را کرد
                """
                ii['pishfactor_id'] = pishfator_number if len(sql_res)>0 else 'پیش فاکتور مبنا یافت نشد لطفا با واحد مالی کنترل شود'
...
**** ****""" % (
        ...
        # اضافه سازی به اسنپ شات برای تغییر ایجاد تغییر پس از امضا
        # dt = HavalehForooshSignsSnapshotSerializer(
        #     data={
        #         'khoroojSignLink': ser.data['id'],
        #         'snapshot': {
        #             'havaleh': HavalehForooshSerializer(instance=approveInstance.havalehForooshLink).data['exp'],
        #             'approve': HavalehForooshApproveSerializer(instance=approveInstance).data['item']['items']
        #         }
        #     }
        #
        # )
