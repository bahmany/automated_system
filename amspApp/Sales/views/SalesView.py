...
PreFactTotalPrice
    ...
    def template_view_pishfactor_base(self, request):
        if request.user.groups.all().filter(name='group_view_pishfactor').count() == 0:
            return render_to_response('Financial/cog/noaccess.html', {},
                                  context_instance=RequestContext(request))
        return render_to_response('Sales/Pishfactor/base.html', {},
                                  context_instance=RequestContext(request))
