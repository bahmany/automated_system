...
angular.module('AniTheme')
    .controller('bpmnSetupCtrl', function ($scope, $http, $q, $translate, $rootScope,
                                           $state, $stateParams, $location, $modal,
                                           bpmnService, $timeout, $mdDialog) {
        ...
        $scope.someThingChanged = function (userSelected) {
            $scope.xx = 1;
        ...
        $scope.setupUserTask = function (userTaskElement) {
            ...
            $scope.bpmn.userTasks.forEach(function (element) {
                if (element.taskId == userTaskElement.id) {
                    ...
                    $scope.chartRank = element.chartRank;
                    $scope.$digest();
                    if ($scope.performerType == 6) {
                        $scope.prePerformer = element.performer;
                    }
        ...
        $scope.saveSetupUserTask = function () {
            ...
            moddle.fromXML($scope.bpmn.xml, function (err, definitions) {
                ...
                $scope.usertask.chartRank = $scope.chartRank;
...
// In this if check if bpmn has any form then update bpmn.form with new data or add new form to it
        ...
        $scope.servicetasks = [];
        $scope.pythonCode = "";
        $scope.setupServiceTask = function (setupElement) {
            // debugger;
            $scope.setupElement = setupElement; // point to selected svg element


            if ($scope.bpmn.otherTasks == null) {
                $scope.bpmn.otherTasks = [];
            }
            $scope.bpmn.otherTasks.forEach(function (element) {
                if (element.taskId == setupElement.id) {
                    $scope.pythonCode = element.pythonCode
                }
            });
            $("#bpmn-viewer").fadeOut(200);
            $('#servicetask-setup-div').fadeIn(200);

        };
        $scope.saveServiceTask = function () {
            // debugger;
            if ($scope.processObjs == null) {
                $scope.processObjs = [];
            }
            ...
            moddle.fromXML($scope.bpmn.xml, function (err, definitions) {
                ...
                // finding service task in bpmn service task list
                // if not found insert
                ...
                $scope.bpmn.otherTasks.forEach(function (value) {
                    if (value.taskId == $scope.setupElement.id) {
                        selectedTaskIndex += 1;
                        found = true;
                        return
                    } else {
                ...
                if (!found) {
                    $scope.bpmn.otherTasks.push({
                        type: "serviceTask",
                        taskId: $scope.setupElement.id,
                    })

                    selectedTaskIndex = $scope.bpmn.otherTasks.length - 1
                }
                //-------------------------------------
                //-------------------------------------
                //-------------------------------------

                $scope.bpmn.otherTasks[selectedTaskIndex].pythonCode = $scope.pythonCode;
                ...
                bpmnService.bpmnUpdate($stateParams.companyid, $scope.bpmn.id, $scope.bpmn).then(function (data) {
                    ...
                    $("#servicetask-setup-div").fadeOut(200);
                    $('#bpmn-viewer').fadeIn(200);
                });
            });
...
//};
        ...
        $scope.cancelServiceTask = function () {
            $("#servicetask-setup-div").fadeOut(200);
            $('#bpmn-viewer').fadeIn(200);
        };
        ...
        viewer.on('element.click', function (event) {
            ...
            if (event.element.type == 'bpmn:ServiceTask') {
                $scope.setupServiceTask(event.element);
            }
        ...
        $scope.performerTypesList = [
            {
                name: 'انتخاب فرد شروع کننده فرآیند توسط سامانه',
                value: 3,
                enabled: true
            },
            {
                name: 'انتخاب سمت ثابت',
                value: 2,
                enabled: true
            },
            {
                name: '  انتخاب شخص ثابت',
                value: 1,
                enabled: true
            },
            {
                name: 'انتخاب سمت بالادستی شروع کننده فرآیند',
                value: 5,
                enabled: true
            },
            {
                name: 'انتخاب سمت بالادستی شروع کننده فرآیند و بالا رفتن از آن با توجه به درجه',
                value: 7,
                enabled: true
            },
            {
                name: 'انتخاب شخص انتخاب شده در فرم های قبلی',
                value: 6,
                enabled: true
            },
            {
                name: 'انتخاب سمت بالادستی شخص انتخاب شده در فرم های قبلی',
                value: 8,
                enabled: false
            },
            {
                name: 'انتخاب سمت بالادستی شخص انتخاب شده در فرم های قبلی و بالارفتن از آن با توجه به درجه',
                value: 9,
                enabled: false
            }
        ]
        ...
        $scope.serviceTypes = [
            ...
            {
                name: "ایجاد وظائف کنترل پروژه",
                value: 4
            },
            {
                name: "ایجاد منابع کنترل پروژه",
                value: 5
            },
            {
                name: "ایجاد وظائف کنترل پروژه",
