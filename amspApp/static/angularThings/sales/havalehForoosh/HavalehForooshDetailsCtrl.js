...
angular.module('AniTheme')
    .controller(
        ...
        function ($scope, $window, $mdMenu, $http, $translate, $mdDialog, $state,
            ...
            $scope.get = function () {
                $http.get("/api/v1/havakehForoosh/" + $stateParams.hfdid + "/getDetails/").then(function (data) {
            ...
            $scope.next_rec = function () {
                $http.get("/api/v1/havakehForoosh/" + $stateParams.hfdid + "/nextRecord/").then(function (data) {
                    if (data.data.id !== "") {
                        $state.go("HavalehForooshDetails", {"hfdid": data.data.id});

                    }
                })
            }
            $scope.prev_rec = function () {
                $http.get("/api/v1/havakehForoosh/" + $stateParams.hfdid + "/prevRecord/").then(function (data) {
                    if (data.data.id !== "") {
                        $state.go("HavalehForooshDetails", {"hfdid": data.data.id});
            ...
            $scope.proveByTolid = function (convItem, event) {
                ...
                $http.post("/api/v1/havakehForoosh/proveTolid/", convItem).then(function (data) {
            ...
            $scope.changeTarikheTahvil = function (approve, item) {
                ...
                if (b) {
                    $http.post("/api/v1/havakehForoosh/changesayer/", {
                        item: item,
                        approve: approve
                    }).then(function (data) {
            ...
            $scope.tolidBelaApply = function (approve, what) {
                $http.post("/api/v1/havakehForoosh/tolidBelaApply/", {
                    item: what,
                    approve: approve
                }).then(function (data) {
            ...
            $scope.khroojBelaApply = function (approve, what) {
                $http.post("/api/v1/havakehForoosh/khroojBelaApply/", {
                    item: what,
                    approve: approve
                }).then(function (data) {
                    if (data.data.type === "err") {
            ...
            $scope.ersaleFactorApply = function (approve, what) {
                $http.post("/api/v1/havakehForoosh/ersaleFactorApply/", {
                    item: what,
                    approve: approve
                }).then(function (data) {
                    if (data.data.type === "err") {
            ...
            $scope.changeKarbod = function (approve, item) {
                ...
                if (b) {
                    $http.post("/api/v1/havakehForoosh/changekarbord/", {
                        item: item,
                        approve: approve
                    }).then(function (data) {
            ...
            $scope.changeHavaleh = function (approve, signID, ev) {
                $state.go("HavalehForooshChange", {ApproveID: approve.id, Step: signID});
            };


            $scope.printIt = function () {
                var cdd = jQuery("html").html();
                $http.post("/api/v1/havakehForoosh/sendAutomated_tolid/", {dt: cdd});
            }
            ...
            $scope.showAdvanced = function (approve, stepID, ev) {
                ...
                $mdDialog.show({
                    ...
                    .then(function (answer) {
                        ...
                        $http.get('/api/v1/havakehForoosh/' + approve.id + '/getDetailBarnamehSigns/').then(function (data) {
            ...
            $scope.weProduceIt = function (approve, $event) {
                $scope.waitForAppr = true;
                $http.post("/api/v1/havakehForooshConv/weProduceIt/", {
                    "approve": approve
                }).then(function (data) {
                    $scope.waitForAppr = false;
            ...
            $scope.readyToSend = function (approve, $event) {
                $scope.waitForAppr = true;
                $http.post("/api/v1/havakehForooshConv/readyToSend/", {
                    "approve": approve
                }).then(function (data) {
                    $scope.waitForAppr = false;
                    approve.convs.push(data.data);
            ...
            function AddNewConvController($scope, $mdDialog, $stateParams, approve) {
                ...
                $scope.postConv = function () {
                    ...
                    if ($scope.conv.id) {
                        $http.patch("/api/v1/havakehForooshConv/" + approve.id + "/", $scope.conv).then(function (data) {
                        ...
                        $http.post("/api/v1/havakehForooshConv/", $scope.conv).then(function (data) {
            ...
            $scope.isThisApproveFinishedTotaly = function () {
                $http.get("/api/v1/havakehForoosh/" + $stateParams.hfdid + "/isThisApproveFinishedTotaly/").then(function (data) {
            ...
            $scope.startNewActivity = function () {
                $http.get("/api/v1/havakehForoosh/" + $stateParams.hfdid + "/startFromRemaining/").then(function (data) {
                    $scope.get();
                    $scope.isThisApproveFinishedTotaly();
                })
            }
            ...
            $scope.listConv = function () {
                return
                $http.get("/api/v1/havakehForooshConv/?convID=" + $stateParams.hfdid).then(function (data) {
            ...
            $scope.deleteConv = function (ev, conv) {
                ...
                $mdDialog.show(confirm).then(function (result) {
                    $http.delete("/api/v1/havakehForooshConv/" + conv.id + "/").then(function () {
                        $scope.listConv();
                    })
                }, function () {
            ...
            $scope.listReplays = function (item) {
                $http.get("/api/v1/havakehForooshConv/" + item.havalehForooshLink + "/getReplays/").then(function (data) {
            ...
            $scope.removeReplay = function (parentItem, item, ev) {
                ...
                var confirm = $mdDialog.confirm()
                    ...
                    .cancel('انصراف');
                $mdDialog.show(confirm).then(function (result) {
                    $http.get("/api/v1/havakehForooshConv/removeReplay/?replayID=" + item.id).then(function () {
            ...
            $scope.replayConv = function (ev, conv) {
                ...
                $mdDialog.show(confirm).then(function (result) {
                    $http.post("/api/v1/havakehForooshConv/" + $stateParams.hfdid + "/AddToReplay/", {
                        comment: result.data
                    }).then(function (data) {
