    ...
    .controller(
        ...
        function ($scope, $window, $http,
            ...
            $scope.gotoprev = function () {
                $http.get('/api/v1/exits/' + $stateParams.khid + '/gotoprev/').then(function (data) {
                    if (data.data.id !== "") {
            ...
            $scope.gotonext = function () {
                $http.get('/api/v1/exits/' + $stateParams.khid + '/gotonext/').then(function (data) {
                    if (data.data.id !== "") {
                        $state.go("KhoroojDetails", {"khid": data.data.id});
            ...
            $scope.getDetails = function () {
                $http.get('/api/v1/exits/' + $stateParams.khid + '/').then(function (data) {
                    $scope.headerDet = data.data;
                    $scope.checkIfSMSNo();
                    $scope.checkIfProfile();
                });

                $http.get('/api/v1/exits/' + $stateParams.khid + '/getDetailBarnamehSigns/').then(function (data) {
                    $scope.signs = data.data;
                });


                $http.get('/api/v1/exits/' + $stateParams.khid + '/getDetailBarnameh/').then(function (data) {
                    ...
                    $http.get('/api/v1/exits/' + $stateParams.khid + '/getFiles/').then(function (data) {
            ...
            $scope.sendSms = function () {
                var elemt = angular.element(document.querySelector('#smsBody'));
                $http.post('/api/v1/exits/' + $stateParams.khid + '/sendSms/', {
                    sms: elemt.text(),
                    mobile: $scope.sms.mobile,
                    withSMS: true,
                    VchHdrId: $scope.headerDet.item.VchHdrId
                }).then(function (data) {
                    $scope.getSMSList();
                })
            }
                ...
                $mdDialog.show({
                    locals: {
                        dataToSend: {
                            ...
                            stepID: stepID,
                            signType: 1,
                            httppost: '/api/v1/exits/signExit/',
                            VchHdrId: $scope.headerDet.item.VchHdrId
                        }
                    },
            ...
            $scope.getSMSList = function () {
                $http.get('/api/v1/exits/' + $stateParams.khid + '/getSMS/').then(function (data) {
            ...
            $scope.RemoveFromUploaded = function (ev, index) {
                ...
                $mdDialog.show(confirm).then(function (result) {
                    ...
                    $http.post("/api/v1/exits/" + $stateParams.khid + "/saveFiles/", {
                        Files: {
