...
from amspApp.amspUser.models import BasicAuths, MyUser
...
def _wrap_users(users, request):
    ...
    if token:
        if len(token) > 40:
            userID = BasicAuths.objects.filter(token=token).first()
            # print(userID)
            if userID:
                # print(userID.userID)
                usr = MyUser.objects.get(id=userID.userID)
...
class RedisStore(object):
    ...
    def _get_message_channels(self, request=None, facility='{facility}', broadcast=False,
        ...
        if isinstance(users, (list, tuple)):
            ...
            channels.append(
                '{prefix}user:{0}:{facility}'.format(request.user.get_username(), prefix=prefix, facility=facility))
        ...
        if isinstance(sessions, (list, tuple)):
            ...
            channels.append(
                '{prefix}session:{0}:{facility}'.format(request.session.session_key, prefix=prefix, facility=facility))
